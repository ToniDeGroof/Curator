...
# Initialiseer de client globaal (bovenaan de app)
...
st.markdown("""
<style>
/* Verberg de tekst "200MB per file · JPG, PNG, WEBP" */
...
st.markdown("""
<style>
/* Verklein de bovenruimte van de hele Streamlit body */
.main {
    padding-top: 0rem !important;
}

/* Verwijder extra witruimte bovenaan de tabs */
.stTabs {
    margin-top: -30px !important;
}

/* Optioneel: verwijder witruimte bovenaan de header */
.block-container {
...
def generate_pdf_report(titel, module, genre, locatie, datum, image_bytes, result_json):
    ...
    story.append(Paragraph(f"<b>CURATOR EVALUATIERAPPORT</b>", title_style))
...
with tab1:
    ...
    with st.expander("Voorwaarden & eisen voor upload", expanded=True):
        st.markdown("""
        **Richtlijnen voor de beste analyse:**
        * **Bestandstypen:** JPG of JPEG, PNG, WEBP
        * **Bestandsgrootte:** Maximaal 10 MB per foto
        * **Aantal beelden:** Maximaal 20 fotos per upload-sessie
        * **Resolutie:** Aanbevolen minimaal 1080 px aan de kortste zijde voor een nauwkeurige beoordeling van compositie en detail
        * **Kwaliteit:** 75-80
        * **Kleurruimte:** Best sRGB voor een getrouwe weergave van licht en kleur bij schermweergave
        """)
    ...
    uploaded_files = st.file_uploader(
        "Kies of sleep je foto's in het oplaadvak",
        ...
        if st.session_state.current_index >= total_files:
            ...
            with col_inputs:
                ...
                with col_f1:
                    ...
                    genre_lijst = ["Straatfotografie", "Portret", "Landschap", "Documentair", "Architectuur", "Stilleven", "Abstract", "Experimenteel", "Overig"]
...
with tab2:
    ...
    st.title("Selectie Studio")

    # CSS OM DE PIJLTJES VAN ST.EXPANDER EN ST.POPOVER TE VERBERGEN, TEKST TE CENTREREN EN WITTE KNOPPEN LICHTGRIJS TE MAKEN
    st.markdown("""
    <style>
        ...
        /* 3. WITTE KNOPPEN (SECUNDAIR) LICHTGRIJS MAKEN */
    ...
    if "curator_search_query" not in st.session_state:
        st.session_state.curator_search_query = ""

    # HULPFUNCTIES VOOR VEILIGE DATAPARSING
    ...
    try:
        ...
        if not all_items:
            ...
            existing_modules = ["Alle modules"] + sorted(list({item.get("module") for item in all_items if item.get("module")}))
            existing_locations = ["Alle locaties"] + sorted(list({item.get("locatie") for item in all_items if item.get("locatie")}))
            existing_dates = ["Alle datums"] + sorted(list({item.get("datumcode") for item in all_items if item.get("datumcode")}))
            existing_genres = ["Alle genres"] + sorted(list({item.get("genre") for item in all_items if item.get("genre")}))
            ...
            with c_top_1:
                ...
                search_query = search_input.lower().strip()
            ...
            search_kleur = None
            if any(w in search_query for w in ["zwart-wit", "zwart wit", "monochroom"]):
            ...
            for item in all_items:
                ...
                p_dict = safe_json_parse(item.get("artistiek_profiel"))
            ...
            # RESULTATENBALK & WERKSET ACTIES
            basket_count = len(st.session_state.curator_basket)
            
            with st.container(border=True):
                c_res_head, c_res_actions = st.columns([0.40, 0.60])
                
                with c_res_head:
                    st.markdown(f"### Selectie: **{len(filtered_items)}** van **{len(all_items)}** beelden")
                    st.caption(f"🛒 Actieve Werkset: **{basket_count}** foto('s) verzameld")

                with c_res_actions:
                    col_act1, col_act2, col_act3 = st.columns([0.38, 0.31, 0.31])
                    
                    with col_act1:
                        if st.button("➕ Voeg alle toe", use_container_width=True, disabled=not filtered_items):
                            for item_tuple in filtered_items:
                                itm = item_tuple[0]
                                st.session_state.curator_basket[itm.get("id")] = itm
                            st.success(f"{len(filtered_items)} foto's toegevoegd!")
                            st.rerun()

                    with col_act2:
                        if st.button("Werkset leegmaken", use_container_width=True, disabled=(basket_count == 0)):
                            st.session_state.curator_basket.clear()
                            st.rerun()

                    with col_act3:
                        if st.button("Naar Studio Curator", type="primary", use_container_width=True, disabled=(basket_count == 0)):
            ...
            if not filtered_items:
                ...
                for i in range(0, len(filtered_items), cols_per_row):
                    ...
                    for idx, (item, p_dict, is_eyecatcher) in enumerate(row_batch):
                        ...
                        deel = safe_json_parse(item.get("deelscores") or p_dict.get("deelscores"))
                            ...
                            with st.container(border=True):
                                ...
                                with st.popover("⚙️ Beheer foto", use_container_width=True):
                                    ...
                                    with st.expander("Bekijk deelscores & profiel", expanded=False):
                                        ...
                                        ld_score = int(round(parse_score(formal.get("light_drama", 0))))
                                        mel_score = int(round(parse_score(emotional.get("melancholy", 0))))
                                        int_score = int(round(parse_score(emotional.get("intimacy", 0))))
                                    ...
                                    if st.button("Verberg in deze weergave", key=f"hide_cur_{item_id}", use_container_width=True):
                                        st.session_state.excluded_curator_ids.add(item_id)
                                        st.rerun()

                                    # 4. DEFINITIEF VERWIJDEREN
                                    with st.expander("Wis uit database"):
...
with tab3:
    ...
    if not basket_items:
        ...
        curator_tab1, curator_tab2 = st.tabs(["AI Curatie Wizard", "Huidige Werkset Overzien"])
...
"""
                        ...
                        try:
                            ...
                            for sel in ai_result.get("selected_items", []):
                                ...
                                if orig:
                                    ...
                                    item_combined["role_tag"] = sel.get("role_tag", "Wandbeeld")
                ...
                if "curated_selection" in st.session_state and st.session_state["curated_selection"]:
                    ...
                    st.markdown("---")
                    st.markdown("### Expositiewand")
                    st.caption("**Schuif de beelden met je muis naar links of rechts** om de gewenste volgorde aan de muur aan te passen!")
                    ...
                    st.markdown("---")
                    st.markdown("### Curator Rapport & Motivering")
                    
                    if st.session_state.get("curator_concept"):
                        st.info(f"**Curator Visie & Concept:** {st.session_state['curator_concept']}")

                    st.success(f"Selectie voltooid! We hebben de **{len(sorted_items)} beste beelden** gekozen uit je werkset van {len(basket_items)} foto's.")
                    ...
                    for s_item in sorted_items:
                        ...
                        with st.container(border=True):
                            ...
                            with col_i2:
                                role_tag = s_item.get("role_tag", "Wandbeeld")
